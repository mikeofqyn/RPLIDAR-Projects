...
class LidarData {
  ...
  public void set(float angle, float distance, int quality, long sequence) {
    ...
    lastseq = sequence;
    float adjusted = distance * (quality / 15.0);
    int i = getangle(angle);
    data[i].addDist(adjusted);
  }
...
void draw() {
  ...
  float rads = (float) Math.toRadians(1.0* angle);

  // display scanning position
  strokeWeight(6);
  float rads2 = (float) Math.toRadians(angle+1);
  float rads1 = (float) Math.toRadians(angle-1);
...
void keyPressed() {
  switch(key) {
    ...
    case 'H':
      ...
      println("h/?    help");
      println("c      clear screen and data");
