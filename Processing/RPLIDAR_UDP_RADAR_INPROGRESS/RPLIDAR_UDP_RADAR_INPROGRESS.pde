...
final int divisionsPerDegree = 10; // Number of divisions per degree = size of points array
...
class LidarData {
  private AngleData[] data = new AngleData[360*divisionsPerDegree];
  ...
  private int getIndex(float angle) {
    int iangle = (int) Math.round(angle*divisionsPerDegree); 
    if (angle<0) iangle = 0;
    if (iangle>=360*divisionsPerDegree) iangle = (360*divisionsPerDegree)-1;
    return iangle;
  }
...
static final color[] coltab = new color[] { #302000, #602020, #802020, #A02020, #F02020, #FF4020, #FF8020, #FFA020, #FFC020, #FFF020, #FFFF20, #FFFF40, #FFFF80, #FFFFA0, #FFFFFF };
...
void draw() {
  ...
  while (!newturn) {
    ...
    float curr_angle = division_n/divisionsPerDegree;
  
    division_n += 1;
    if (division_n >= (360.0*divisionsPerDegree)-1) {
      division_n = 0;
      newturn = !newturn;
    }
  
    boolean isDirty = sharedData.get(curr_angle, dd, eraseData);
    long  np = dd.npoints;
  
    float rads = (float) Math.toRadians(curr_angle);
  
    // display scanning position
  
    float rads2 = (float) Math.toRadians(curr_angle+(1.0/divisionsPerDegree));
    float rads1 = (float) Math.toRadians(curr_angle-(1.0/divisionsPerDegree));
    int ncolor = newturn? 100: 180;
    int rcolor = isDirty? 255: 0;
